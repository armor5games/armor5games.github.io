...
if (jQuery("body").fancybox == null) { // check if fancybox script is loaded
...
function triggerCerchezLightboxGalleryInit(target) {
	jQuery('.gallery:not(.gallery-init)', target).each(function() {
		$this = jQuery(this);
		if ($this.hasClass('gallery-init')) return;
...
function triggerCerchezLightboxInit(target) {
	// disable lightbox on small devices
	if (window.innerWidth < 500 && window.innerHeight < 500) return;

	triggerCerchezLightboxGalleryInit(target);

	jQuery('a.lightbox, .gallery-item a', target).fancybox({
		padding    : 4,
		margin     : [60, 10, 10, 10],
		maxHeight  : '90%',
		maxWidth   : '90%',
		...
		beforeShow : function() {
			if (this.element.parents('.gallery-item').length > 0) {
...
jQuery(function() {
	triggerCerchezLightboxInit(document);
});

jQuery(document).on("cerchezWidgetUpdate", function(e){
	triggerCerchezLightboxGalleryInit(e.target);
});
